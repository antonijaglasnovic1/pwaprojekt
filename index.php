...
<main class="container main">
    <section class="section" id="celebrities">
        <h2 class="section-title">Celebrities</h2>
        <div class="row">
            <?php 
                $query = "SELECT * FROM jutarnji WHERE arhiva=0 AND kategorija='celebrities' LIMIT 6"; 
                ...
                if (mysqli_num_rows($result) > 0) {
                    while ($row = mysqli_fetch_array($result)) { 
                        echo '<div class="col-lg-4 col-md-6 col-sm-12 article">';
                    ...
                    echo "Nema vijesti u kategoriji celebrities.";
                }
            ?>
        </div>
    </section>
    ...
    <section class="section" id="makeup">
        ...
        <div class="row">
            <?php 
                $query = "SELECT * FROM jutarnji WHERE arhiva=0 AND kategorija='makeup' LIMIT 3"; 
                $result = mysqli_query($dbc, $query); 
                if (!$result) {
                    die("Query failed: " . mysqli_error($dbc));
                }
                if (mysqli_num_rows($result) > 0) {
                    ...
                    echo "Nema vijesti u kategoriji makeup.";
                }
            ?>
        </div>
    </section>
